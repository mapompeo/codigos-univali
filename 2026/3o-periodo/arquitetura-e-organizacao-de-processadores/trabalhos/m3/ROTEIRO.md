...
Bom dia / boa tarde. O trabalho que vamos apresentar é a implementação de um processador completo do zero, em VHDL.
...
O processador é dividido em dois grandes blocos. O primeiro é o datapath, que é por onde os dados circulam: registradores, ULA, memórias. O segundo é a unidade de controle, que lê a instrução e decide o que cada componente deve fazer. Juntos, eles formam o circuito que vamos mostrar agora.

*[abrir o diagrama do datapath]*
...
O **RegFile** é o banco com 32 registradores de 32 bits, de x0 a x31. O x0 é especial — ele é hardwired zero, qualquer escrita nele é descartada e qualquer leitura retorna zero. A leitura é combinacional, ou seja, instantânea. A escrita acontece na borda de subida do clock.
...
A **ULA** executa dez operações: ADD, SUB, AND, OR, XOR, SLL, SRL, SRA, SLT e SLTU. Ela também produz um sinal chamado zero, que fica em 1 quando o resultado é zero — esse sinal é usado pela lógica de branch para decidir se o desvio é tomado ou não.

A **DataMem** é a memória de dados, usada pelas instruções LW e SW. O endereço vem do resultado da ULA, calculado como rs1 mais o imediato. A leitura é combinacional, a escrita é síncrona.
...
A prova definitiva de que o branch funcionou é que x12 permanece zero ao final da simulação. Se o desvio não tivesse sido tomado, x12 teria recebido 99.

A simulação foi feita no EDA Playground com o GHDL. No waveform é possível ver o PC pulando de 32 direto para 40, o sinal zero ficando em 1 no ciclo do BEQ — confirmando que 6 menos 6 é zero — e x13 recebendo 1 no ciclo seguinte, que é a instrução logo após o salto. O projeto está disponível no repositório com todos os arquivos VHDL, o testbench e um manual detalhado. Obrigado.

---

> **Dica:** Pessoa 1 abre o diagrama antes de começar e deixa na tela.  
> Pessoa 3 pode mostrar o EDA Playground ou o waveform ao falar dos resultados.
