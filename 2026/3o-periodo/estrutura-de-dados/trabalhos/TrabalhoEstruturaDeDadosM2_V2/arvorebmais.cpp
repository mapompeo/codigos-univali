...
{
    int i = 0;
    while (i < no->tamanhoChaves && chave > no->chaves[i])
        i++;
    ...
    if (no->folha)
    {
        ...
        {
            if (no->chaves[k] != chave)
                no->chaves[j++] = no->chaves[k];
        }
        no->tamanhoChaves = j;
        return;
    }
...
{
    while (!no->folha)
        no = no->filhos[no->tamanhoFilhos - 1];
...
No *ArvoreBMais::encontrarPai(No *filho)
{
    ...
    while (!atual->folha)
    {
        ...
        {
            if (filho == atual->filhos[i])
                return atual;
            else if (filho->chaves[0] < atual->filhos[i]->chaves[0])
                break;
            i++;
        }
        atual = atual->filhos[i];
